...
import { BACKGROUND_PRESETS, Shape, Layer, ViewMode, GizmoMode, BackgroundPreset } from './store/useStudioStoreV2';
...
const PackageEditorPageV2: React.FC = () => {
  ...
  const loadShape = useCallback((
    ...
    if (sceneRef.current) {
      ...
      const toRemove = sceneRef.current.children.filter(c => !systemNames.includes(c.name) && !(c === (transformRef.current as any)));
      toRemove.forEach(c => {
  ...
  const saveDesign = async () => {
    ...
    try {
      ...
      const res = await fetch('https://www.pouch.eco/api/save-design', {
        method: 'POST',
  ...
  const createCustomLink = async () => {
    ...
    try {
      ...
      const res = await fetch('https://www.pouch.eco/api/save-custom-studio', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
