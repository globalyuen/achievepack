...
EP_MAIN = "/Users/ryanmacmini/Desktop/1 App i made/Master Achieve Pack/pouch-eco-website/src/main.tsx"
...
def patch_main(filepath):
    ...
    if imports_to_add:
        # Find a good place to inject, e.g. after the last lazyWithRetry
        match = re.search(r"const \w+Page = lazyWithRetry[^\n]+\n", content)
        if match:
    ...
    if routes_to_add:
        # Find </Routes> and inject before it, or inside a specific group
        # Let's inject just before <Route path="*" element={<Navigate to="/" replace />} />
...
patch_main(AP_MAIN)
patch_main(EP_MAIN)
print("Patched both main.tsx files.")
