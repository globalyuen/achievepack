...
async function run() {
  ...
  const pattern = /ass="text-\[#10b981\] font-bold hover:underline"/g
  if (pattern.test(postStr)) {
    ...
    contentStr = contentStr.replace(/ass="text-\[#10b981\] font-bold hover:underline"/g, 'class="text-[#10b981] font-bold hover:underline"')
    ...
    console.log('No matches of "ass="text-[#10b981] font-bold hover:underline"" found in post.content.')
    ...
    for (const [key, val] of Object.entries(post)) {
      if (typeof val === 'string' && val.includes('ass="text-[#10b981]')) {
        ...
        updatedFields[key] = val.replace(/ass="text-\[#10b981\] font-bold hover:underline"/g, 'class="text-[#10b981] font-bold hover:underline"')
        found = true
      }
    }
