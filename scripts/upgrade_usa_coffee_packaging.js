...
const BRAND_SEEDS = ["DailyGrind Roasters", "Roastwood Coffee", "BloomBrews", "RoastCraft"];
const COUNTRY_SEEDS = ["USA", "Germany", "United Kingdom", "Canada"];
const SHAPE_SEEDS = ["Stand up pouch", "Flat bottom pouch", "Gusset pouch"];
const CLOSURE_SEEDS = ["one sided zipper", "regular zipper", "tin tie"];
...
async function run() {
  const slug = 'usa-coffee-packaging';
  const keyword = 'usa coffee packaging';
  
  const brand = BRAND_SEEDS[0];
  const country = COUNTRY_SEEDS[0];
  const shape = SHAPE_SEEDS[0];
  const closure = CLOSURE_SEEDS[0];
  const material = COMPOSTABLE_MATERIALS[0];
  const product = "specialty roasted coffee beans";

  console.log(`⚙️ Upgrading slug: "${slug}"`);

  // Generate pouch and achievepack versions
  ...
  const templateName = 'coffee-packaging-guide';
  for (let i = 1; i <= 6; i++) {
    const srcName = i === 1 
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      image_url: '/imgs/blog/heros/usa-coffee-packaging-hero.png',
  ...
  if (error) {
    ...
    console.log('✅ Successfully upgraded usa-coffee-packaging!');
  }
  process.exit(0);
}
