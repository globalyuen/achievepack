import React from 'react'
import { useTranslation } from 'react-i18next'
import { Leaf, Beaker, Zap, Layers, BookOpen, MessageCircle, Calendar, Download, Mail, CheckCircle } from 'lucide-react'
import SEOPageLayout from '../../components/SEOPageLayout'
import ClickableImage from '../../components/ClickableImage'
import { useCalendly } from '../../contexts/CalendlyContext'

const FreshProducePage: React.FC = () => {
  const { t } = useTranslation()
  const { openCalendly } = useCalendly()
  const p = 'seoPages.pages.freshProduce'

  const getTranslationArray = <T = string,>(key: string): T[] => {
    const val = t(key, { returnObjects: true })
    return Array.isArray(val) ? (val as T[]) : []
  }

  const sections = [
    {
      id: 'mockup',
      title: 'Anti-Fog Micro-Perforated Pouch Showcase',
      icon: <Leaf className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700">
          <p className="text-sm text-neutral-600">Click the product image below to expand:</p>
          <div className="flex justify-center">
            <ClickableImage 
              src="/imgs/knowledge/fresh-produce-pouch-arugula.png" 
              alt="Fresh Produce Packaging - Breathable Anti-Fog Pouch for Arugula and Leafy Greens" 
              className="max-w-full md:max-w-xl rounded-lg shadow-lg border border-neutral-200 cursor-pointer hover:shadow-xl transition"
              caption="Custom micro-perforated bag with anti-fog window for maximum freshness and visibility"
            />
          </div>
        </div>
      )
    },
    {
      id: 'respiration',
      title: t(`${p}.sections.respiration.title`),
      icon: <Leaf className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700 leading-relaxed">
          <p>{t(`${p}.sections.respiration.desc`)}</p>
        </div>
      )
    },
    {
      id: 'perforations',
      title: t(`${p}.sections.perforations.title`),
      icon: <Zap className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700 leading-relaxed">
          <p>{t(`${p}.sections.perforations.desc`)}</p>
        </div>
      )
    },
    {
      id: 'antifog',
      title: t(`${p}.sections.antifog.title`),
      icon: <Beaker className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700 leading-relaxed">
          <p>{t(`${p}.sections.antifog.desc`)}</p>
        </div>
      )
    },
    {
      id: 'materials',
      title: t(`${p}.sections.materials.title`),
      icon: <Layers className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700 leading-relaxed">
          <p>{t(`${p}.sections.materials.desc`)}</p>
        </div>
      )
    },
    {
      id: 'notebook',
      title: t(`${p}.sections.notebook.title`),
      icon: <BookOpen className="h-5 w-5 text-primary-600" />,
      content: (
        <div className="space-y-4 text-neutral-700 leading-relaxed">
          <p className="italic bg-neutral-50 border-l-4 border-primary-500 p-4 rounded-r-lg font-mono text-sm text-neutral-800">
            {t(`${p}.sections.notebook.desc`)}
          </p>
        </div>
      )
    }
  ]

  const faqs = [
    { question: t(`${p}.faqs.q1`), answer: t(`${p}.faqs.a1`) },
    { question: t(`${p}.faqs.q2`), answer: t(`${p}.faqs.a2`) },
    { question: t(`${p}.faqs.q3`), answer: t(`${p}.faqs.a3`) },
    { question: t(`${p}.faqs.q4`), answer: t(`${p}.faqs.a4`) },
    { question: t(`${p}.faqs.q5`), answer: t(`${p}.faqs.a5`) }
  ]

  const tables = [
    {
      title: t(`${p}.table.title`),
      data: {
        headers: getTranslationArray(`${p}.table.headers`),
        rows: getTranslationArray<string[]>(`${p}.table.rows`)
      }
    }
  ]

  const relatedLinks = [
    { title: "Compostable Packaging", url: "/materials/compostable", description: "Certified EN 13432 compostable bags for organic brands" },
    { title: "Recyclable Mono-PE", url: "/materials/recyclable-mono-pe", description: "100% PE bags recyclable in standard streams" },
    { title: "Snack & Food Packaging", url: "/industry/snacks-food", description: "High barrier packaging solutions for retail foods" }
  ]

  return (
    <SEOPageLayout
      heroBgColor="#15803d"
      title={t(`${p}.title`)}
      description={t(`${p}.description`)}
      keywords={[
        'fresh produce packaging',
        'arugula packaging bags',
        'spinach bags',
        'kale packaging',
        'anti fog packaging film',
        'micro perforated bags',
        'breathable produce bags',
        'custom vegetable packaging',
        'salad greens pouch',
        'VFFS produce packaging'
      ]}
      canonicalUrl="https://achievepack.com/industry/fresh-produce"
      heroTitle={t(`${p}.heroTitle`)}
      heroSubtitle={t(`${p}.heroSubtitle`)}
      heroImage="/imgs/knowledge/fresh-produce-pouch-arugula.png"
      heroImageAlt="Breathable Micro-Perforated and Anti-Fog Produce Pouch"
      introSummary={t(`${p}.introSummary`)}
      sections={sections}
      faqs={faqs}
      tables={tables}
      schemaType="Product"
      relatedLinks={relatedLinks}
      ctaTitle={t(`${p}.ctaTitle`)}
      ctaDescription={t(`${p}.ctaDescription`)}
      ctaButtonText={t(`${p}.ctaButtonText`)}
    />
  )
}

export default FreshProducePage
