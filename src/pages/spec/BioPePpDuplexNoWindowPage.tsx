...
import SortableMaterialStructuresTable, { BIO_PE_STRUCTURES } from '../../components/SortableMaterialStructuresTable'
import { useTranslation } from 'react-i18next'

const BioPePpDuplexNoWindowPage: React.FC = () => {
  const { t } = useTranslation()
  const p = 'seoPages.pages.bioPePpDuplexNoWindow'
      ...
      content: (
        <div className="space-y-4 text-neutral-700">
          <div className="bg-purple-50 p-6 rounded-lg border border-purple-200">
            <ClickableImage
              src="/imgs/spec/bio-pe-pp-duplex-no-window.webp"
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm text-purple-600">{t(`${p}.sections.structureOverview.badge`)}</span>
            </div>
            <div className="grid sm:grid-cols-2 gap-4 mt-4">
              ...
              <div className="bg-white p-3 rounded-lg">
                ...
                <p className="font-semibold text-purple-700">{t(`${p}.sections.structureOverview.layersValue`)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 text-neutral-700">
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-4 bg-purple-50 rounded-lg">
              ...
              <div>
                <p className="font-semibold text-purple-800">{t(`${p}.sections.layerBreakdown.layer1.name`)}</p>
                <p className="text-sm text-purple-700">{t(`${p}.sections.layerBreakdown.layer1.desc`)}</p>
              </div>
            </div>
      ...
      content: (
        <div className="space-y-4 text-neutral-700">
          ...
          <div className="bg-purple-50 p-4 rounded-lg mt-4">
            <h4 className="font-semibold text-purple-800 mb-2">{t(`${p}.sections.barrierProperties.premiumShelf`)}</h4>
            <p className="text-sm text-purple-700">{t(`${p}.sections.barrierProperties.premiumShelfDesc`)}</p>
          </div>
        </div>
      )
    },
      ...
      content: (
        <div className="space-y-4 text-neutral-700">
          ...
          <SortableMaterialStructuresTable
            ...
            title="All Bio-PE Structures – Compare & Filter"
            categoryColor="green"
          />
        </div>
      )
  ...
  const relatedLinks = [
    ...
    { title: t(`${p}.relatedLinks.link2.title`), url: "/spec/biope-pp-duplex-clear", description: t(`${p}.relatedLinks.link2.description`) },
    { title: t(`${p}.relatedLinks.link3.title`), url: "/spec/biope-pet-duplex-nowindow", description: t(`${p}.relatedLinks.link3.description`) },
    { title: t(`${p}.relatedLinks.link4.title`), url: "/spec/biope-pp-triplex-metalised", description: t(`${p}.relatedLinks.link4.description`) },
  ]
