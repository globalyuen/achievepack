...
import { 
  ...
  Coffee, Apple, Package as PackageIcon, ShoppingCart
} from 'lucide-react'
import { supabase, MiniSite, MiniSiteAccess, MiniSiteContent, Profile } from '../../lib/supabase'

// Default content templates for different industries
const DEFAULT_CONTENT_TEMPLATES: Record<string, MiniSiteContent> = {
  coffee: {
    brand: {
      name: 'Coffee Shop',
      tagline: 'Fresh & Organic',
      taglineWords: ['Fresh', 'Organic', 'Quality'],
      description: 'Premium coffee experience for discerning taste.',
      ctaText: 'Shop Now',
      ctaLink: '#shop'
    },
    hero: {
    ...
    mission: {
      title: 'Our Mission',
      content: 'We are dedicated to sourcing the finest coffee beans and delivering exceptional experiences.',
      image: 'https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?w=1200&q=80',
      coordinates: '0° N — 0° E'
    },
    subscription: {
      title: 'Never Run Out',
      steps: ['Choose', 'Subscribe', 'Enjoy'],
  ...
  food: {
    brand: {
      name: 'Organic Foods',
      tagline: 'Pure & Healthy',
      taglineWords: ['Pure', 'Healthy', 'Natural'],
      description: 'Organic food products for a healthier lifestyle.',
      ctaText: 'Explore Products',
      ctaLink: '#products'
    },
    hero: {
      backgroundImage: 'https://images.unsplash.com/photo-1490885578174-acda8905c224?w=1920&q=80',
      overlayOpacity: 0.4
    },
    marquee: {
      text: 'ORGANIC * FRESH * HEALTHY',
      speed: 30
    },
    products: [
      {
        ...
        origin: 'Local Farm',
        process: 'Organic',
        image: 'https://images.unsplash.com/photo-1569718212165-3a826dbfccf3?w=600&q=80'
      }
    ],
    collections: [
      {
    ...
    mission: {
      ...
      content: 'Providing organic, locally sourced food products for a healthier community.',
      image: 'https://images.unsplash.com/photo-1493770348161-360a1ec8adb0?w=1200&q=80',
      coordinates: '0° N — 0° E'
    },
    subscription: {
      title: 'Weekly Delivery',
      steps: ['Choose', 'Subscribe', 'Receive'],
      discount: 'Free delivery on orders over $50'
    }
  },
  packaging: {
    brand: {
      ...
      taglineWords: ['Sustainable', 'Eco', 'Friendly'],
      description: 'Environmentally friendly packaging solutions.',
      ctaText: 'View Solutions',
      ctaLink: '#solutions'
    },
    hero: {
      backgroundImage: 'https://images.unsplash.com/photo-1581578021424-ebdc0b1b0967?w=1920&q=80',
      overlayOpacity: 0.4
    },
    marquee: {
      text: 'ECO-FRIENDLY * SUSTAINABLE * BIODEGRADABLE',
      speed: 30
    },
    products: [
      {
        ...
        origin: 'USA',
        process: 'Compostable',
        image: 'https://images.unsplash.com/photo-1602002600570-6b7f78ba8a40?w=600&q=80'
      }
    ],
    collections: [
      {
        id: 'collection1',
        name: 'Compostable Line',
        logo: 'COMPOSTABLE',
        description: 'Fully compostable packaging options.',
        bgColor: 'from-green-800 to-green-900',
        images: ['https://images.unsplash.com/photo-1581578021424-ebdc0b1b0967?w=400&q=80']
      }
    ],
    mission: {
      title: 'Environmental Impact',
      content: 'Reducing plastic waste through innovative packaging solutions.',
      image: 'https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?w=1200&q=80',
      coordinates: '0° N — 0° E'
    },
    subscription: {
      title: 'Bulk Orders',
      steps: ['Specify', 'Order', 'Receive'],
...
const ImagePickerModal: React.FC<{
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `demo-sites/${Date.now()}-${Math.random().toString(36).substring(7)}.${ext}`
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden">
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(85vh-60px)]">
          ...
          <div className="mb-4 p-4 border-2 border-dashed border-gray-300 rounded-xl hover:border-primary-400 transition-colors">
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileUpload} className="hidden" id="demo-image-upload" />
            <label htmlFor="demo-image-upload" className="flex flex-col items-center justify-center cursor-pointer py-3">
              {isUploading ? (
...
// Main Website Demo CMS Component
const WebsiteDemoCMS: React.FC = () => {
  ...
  const [editContent, setEditContent] = useState<MiniSiteContent>(DEFAULT_CONTENT_TEMPLATES.default)
  const [editMeta, setEditMeta] = useState<{ name: string; slug: string; description: string; is_public: boolean; status: 'draft' | 'published' | 'archived'; template: 'coffee-shop' | 'bakery' | 'retail' | 'custom' | string }>({ 
    name: '', 
    slug: '', 
    description: '', 
    is_public: false, 
  ...
  const createSite = async () => {
    ...
    try {
      ...
      const templateContent = DEFAULT_CONTENT_TEMPLATES[editMeta.template] || DEFAULT_CONTENT_TEMPLATES.default

      const { data, error } = await supabase
        .from('mini_sites')
        .insert({
          name: editMeta.name || 'New Demo Site',
          slug,
          description: editMeta.description,
          template: editMeta.template,
          status: 'draft',
  ...
  const openEditor = (site: MiniSite) => {
    ...
    setEditContent(site.content || DEFAULT_CONTENT_TEMPLATES.default)
    setEditMeta({
      name: site.name,
      slug: site.slug,
    ...
    try {
      ...
      setSites(prev => prev.map(s => 
        s.id === selectedSite.id 
          ? { ...s, name: editMeta.name, slug: editMeta.slug, description: editMeta.description, is_public: editMeta.is_public, status: editMeta.status, content: editContent, template: editMeta.template as 'coffee-shop' | 'bakery' | 'retail' | 'custom' }
          : s
      ))
  ...
  // Template options
  const templates = [
    { id: 'default', name: 'Default', icon: <Globe className="w-4 h-4" /> },
    { id: 'coffee', name: 'Coffee Shop', icon: <Coffee className="w-4 h-4" /> },
    { id: 'food', name: 'Food & Grocery', icon: <Apple className="w-4 h-4" /> },
    { id: 'packaging', name: 'Packaging', icon: <PackageIcon className="w-4 h-4" /> },
  ]
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Website Demo CMS</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white rounded-xl border">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No demo sites yet</h3>
            <p className="text-gray-500 mb-4">Create your first demo website</p>
            <button 
              onClick={() => {
                setEditMeta({
                  name: 'New Demo Site',
              ...
              <div key={site.id} className="bg-white rounded-xl border shadow-sm overflow-hidden hover:shadow-md transition">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
                    {templates.find(t => t.id === site.template)?.icon}
                    <span>{templates.find(t => t.id === site.template)?.name || 'Default'}</span>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={editMeta.slug} 
                  onChange={(e) => setEditMeta(prev => ({ ...prev, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))} 
                  className="w-full px-3 py-2 border rounded-lg text-sm" 
                />
              </div>
              <div>
                ...
                <select 
                  value={editMeta.template} 
                  onChange={(e) => setEditMeta(prev => ({ ...prev, template: e.target.value as 'coffee-shop' | 'bakery' | 'retail' | 'custom' }))} 
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                >
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setEditMeta(prev => ({ ...prev, status: e.target.value as any }))} 
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                >
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input 
                  ...
                  onChange={(e) => setEditMeta(prev => ({ ...prev, is_public: e.target.checked }))} 
                  className="rounded" 
                />
