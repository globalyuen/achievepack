...
const generateICS = (lead: WhatsAppInquiry, dateStr: string, notes: string) => {
  if (!dateStr) return;
...
VERSION:2.0
PRODID:-//AchievePack//WhatsAppFollowUp//EN
BEGIN:VEVENT
SUMMARY:Follow Up: ${lead.name}
DTSTART;VALUE=DATE:${startD}
DTEND;VALUE=DATE:${startD}
DESCRIPTION:【WhatsApp Follow-up】\\nName: ${lead.name}\\nPhone: ${lead.phone}\\n\\nChat: ${lead.chat_history?.slice(0, 100)}\\n\\nNotes: ${notes}
...
export default function WhatsAppFollowUp() {
  ...
  const [editingLead, setEditingLead] = useState<WhatsAppInquiry | null>(null);
  ...
  const fetchInquiries = async () => {
    ...
    try {
      ...
      if (data) {
        setInquiries(data as WhatsAppInquiry[]);
      }
    } catch (err: any) {
    ...
    try {
      const response = await fetch('/api/whatsapp-ai-suggest', {
        ...
        body: JSON.stringify({
          ...
          chatHistory: editingLead.chat_history
        })
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const filteredInquiries = inquiries.filter(lead => {
    ...
    return (
      ...
      (lead.chat_history && lead.chat_history.toLowerCase().includes(term)) ||
  ...
  const groupedByDate = filteredInquiries.reduce((acc, lead) => {
    ...
    return acc;
  }, {} as Record<string, WhatsAppInquiry[]>);
      ...
      <div className="bg-gradient-to-br from-amber-800 to-amber-950 rounded-2xl p-6 mb-8 text-white shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-2xl font-bold mb-1 flex items-center gap-2">
            <TrendingUp className="w-6 h-6 text-amber-300"/> WhatsApp 訊息跟進面板
          </h2>
          <p className="text-amber-100 text-sm mb-6">
            追蹤所有從 OpenClaw 擷取的 WhatsApp 客戶對話，迅速將跟進行動記錄與下載日曆提醒。
          </p>
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-gray-600 text-xs uppercase tracking-wider">
                ...
                <th className="py-3 px-4 font-bold w-[15%]">同步日期</th>
                <th className="py-3 px-4 font-bold w-[35%]">對話歷史 (Chat History)</th>
                  ...
                  <tr key={lead.id} className="hover:bg-amber-50/30 transition-colors bg-white group">
                    ...
                    <td className="py-3 px-4 align-top">
                      <div className="text-[12px] text-gray-600 leading-relaxed max-w-full">
                        <div className="line-clamp-3 hover:line-clamp-none transition-all whitespace-pre-wrap font-mono" title={lead.chat_history}>
                          {lead.chat_history || <span className="text-gray-400 italic">（無對話）</span>}
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4 align-top">
                  ...
                  {groupedByDate[dateStr].map(lead => {
                    ...
                    return (
                      <div key={lead.id} className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="text-[11px] text-gray-500 mb-4 line-clamp-2 min-h-[2rem] font-mono">
                          {lead.chat_history || '（無對話內容）'}
                        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6 border-b pb-4">
              <div>
                ...
                <p className="text-gray-500 text-sm">{editingLead.phone || 'No phone'}</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 text-xs text-amber-900">
                <span className="font-bold">歷史對話：</span>
                <pre className="mt-1 leading-relaxed text-amber-800 whitespace-pre-wrap font-mono max-h-40 overflow-y-auto">{editingLead.chat_history || '（未填寫）'}</pre>
              </div>
                  ...
                  <div className="p-3 bg-gradient-to-r from-amber-50/50 to-orange-50/30 rounded-xl border border-amber-100/60 text-xs space-y-2">
                    ...
                    <div className="mt-2.5">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-semibold text-amber-900">建議回覆草稿 (WhatsApp)：</span>
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(aiSuggestion.whatsappDraft);
                            alert('WhatsApp草稿已複製到剪貼簿！');
                          }}
