...
export const NeoButton = ({ children, onClick, variant = 'primary', className = '', href, to, target }: { children: ReactNode, onClick?: () => void, variant?: 'primary' | 'secondary' | 'dark', className?: string, href?: string, to?: string, target?: string }) => {
...
export const NeoCard = ({ children, className = '', color = 'bg-white' }: { children: ReactNode, className?: string, color?: string }) => (
  <motion.div 
...
export const NeoBadge = ({ children, color = 'lime' }: { children: ReactNode, color?: 'lime' | 'cyan' | 'magenta' | 'amber' | string }) => {
  ...
  return (
    <span className={`inline-block px-3 py-1 text-xs font-black uppercase border-2 border-black ${colorClass} shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]`}>
      {children}
    </span>
  )
}
