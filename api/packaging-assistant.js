...
// ============ RETRIEVAL LAYER ============
function findRelevantProducts(question) {
    ...
    if (q.includes('box') || q.includes('carton') || q.includes('corrugated') || q.includes('tuck') || q.includes('rigid') || q.includes('chocolate')) {
        const boxProducts = PRODUCT_CATALOG.filter(p => p.category === 'boxes' ||
            p.bestFor?.some((b) => ['chocolate', 'gift boxes', 'premium packaging', 'luxury packaging'].includes(b)));
        if (boxProducts.length > 0)
            return boxProducts.slice(0, 5);
    }
    ...
    if (q.includes('coffee') || q.includes('roast') || q.includes('bean') ||
        ...
        const coffeeProducts = PRODUCT_CATALOG.filter(p => p.bestFor?.some((b) => ['coffee', 'tea'].includes(b)) ||
            p.barrier === 'high' ||
            p.description.toLowerCase().includes('coffee'));
        if (coffeeProducts.length > 0)
            return coffeeProducts.slice(0, 5);
    }
    ...
    if (q.includes('eco') || q.includes('sustainable') || q.includes('compostable') ||
        ...
        const ecoProducts = PRODUCT_CATALOG.filter(p => p.category.includes('eco') ||
            p.materials?.some((m) => ['Compostable', 'BioPE', 'PCR', 'Mono PE'].includes(m)));
        if (ecoProducts.length > 0)
            return ecoProducts.slice(0, 5);
    }
    ...
    const scored = PRODUCT_CATALOG.map(product => {
        const searchText = `${product.name} ${product.description} ${product.category} ${product.materials?.join(' ') || ''} ${product.shape || ''} ${product.bestFor?.join(' ') || ''}`.toLowerCase();
        let score = 0;
        keywords.forEach(kw => {
            ...
            if (product.name.toLowerCase().includes(kw))
                score += 2;
            if (product.bestFor?.some((b) => b.includes(kw)))
                score += 3;
        });
...
function findRelevantFAQs(question) {
    const q = question.toLowerCase();
...
function findRelatedPages(question, pageContext) {
    const q = question.toLowerCase();
    const pages = [];
    ...
    return pages
        .filter(p => {
        if (seen.has(p.url) || p.url === currentPath)
            return false;
        seen.add(p.url);
        return true;
    })
...
// ============ xAI INTEGRATION ============
async function callXAI(userQuestion, context, pageContext) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const productContext = relevantProducts.length > 0
            ? `RELEVANT PRODUCTS:\n${relevantProducts.map(p => `- ${p.name}: ${p.description}. Starting from US$${p.basePrice}. MOQ: ${p.moq} pieces.${p.materials ? ` Materials: ${p.materials.join(', ')}` : ''}${p.turnaround ? ` Turnaround: ${p.turnaround}` : ''}`).join('\n')}`
